...
1. **Ключи подписи обновлений.** Без них `tauri-plugin-updater` не примет релиз:
   ...
   Приватный ключ в git не попадает — он в `.gitignore`. Потеря ключа means
   существующие установки перестанут обновляться, поэтому держите копию в
   надёжном месте.

2. **Адрес манифеста обновлений** (`plugins.updater.endpoints`). Два варианта:
   - публичный репозиторий → можно указывать прямо на релизы GitHub:
     `https://github.com/<owner>/<repo>/releases/latest/download/latest.json`;
   - приватный репозиторий → релизы закрыты авторизацией, поэтому `latest.json`
     и инсталлятор нужно выкладывать на свой хост (например
     `https://download.zexorvpn.site/`), куда CI будет их копировать.
